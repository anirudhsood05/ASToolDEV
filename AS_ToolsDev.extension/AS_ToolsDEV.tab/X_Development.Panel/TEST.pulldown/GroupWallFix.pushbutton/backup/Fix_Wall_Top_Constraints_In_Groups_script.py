...
def fix_group_type(gti):
    ...
    #    After ungroup+regroup, the new group inherits the primary's world-
    #    space element positions. We record the primary's rotation/mirror so
    #    we can compute the DELTA transform needed for each other instance.
    ...
    try:
        ploc = primary.Location
        if ploc is not None and hasattr(ploc, "Rotation"):
            primary_rotation = ploc.Rotation
    except Exception:
        pass
    try:
    ...
    for inst in others:
        try:
            ...
            # Rotation angle (radians) from LocationPoint
            rotation = 0.0
            ...
            # Delta rotation: how much to rotate relative to the new group
            # (which inherits the primary's orientation after regroup)
            delta_rotation = rotation - primary_rotation

            # Delta mirror: only need to mirror if this instance's mirror
            # state differs from the primary's
    ...
    #    PlaceGroup creates at the insertion point with the same orientation
    #    as the group type definition (which now matches the primary's
    #    orientation). Apply delta rotation and/or mirror to match each
    #    original instance's orientation.
    placed_count = 0
    mirror_count = 0
    rotate_count = 0
    with revit.Transaction("Re-place group instances"):
        for data in other_data:
            try:
                new_inst = doc.Create.PlaceGroup(data["point"], new_group_type)
                if new_inst is None:
                    continue

                current_id = new_inst.Id
                pt = data["point"]

                # Apply mirror FIRST if needed (mirroring changes handedness,
                # which affects how subsequent rotation is interpreted)
                if data.get("needs_mirror", False):
                    try:
                        # Mirror about a vertical plane through the insertion
                        # point with X-axis normal. MirrorElement creates a
                        # mirrored copy; we then delete the un-mirrored original.
                        mirror_plane = DB.Plane.CreateByNormalAndOrigin(
                            DB.XYZ(1, 0, 0), pt
                        )
                        DB.ElementTransformUtils.MirrorElement(doc, current_id, mirror_plane)
                        doc.Delete(current_id)

                        # Find the newly created mirrored copy (highest Id of
                        # this group type). MirrorElement doesn't return the new
                        # Id, so we re-collect.
                        doc.Regenerate()
                        mirrored_inst = None
                        for g in DB.FilteredElementCollector(doc).OfClass(DB.Group):
                            try:
                                if (g.GroupType is not None
                                        and eid_int(g.GroupType.Id) == eid_int(new_group_type.Id)):
                                    if mirrored_inst is None or eid_int(g.Id) > eid_int(mirrored_inst.Id):
                                        mirrored_inst = g
                            except Exception:
                                continue
                        if mirrored_inst is not None:
                            current_id = mirrored_inst.Id
                        mirror_count += 1
                    except Exception as e:
                        logger.warning("Mirror failed at ({:.1f}, {:.1f}): {}".format(
                            pt.X, pt.Y, str(e)))

                # Apply delta rotation if non-zero
                delta_rot = data.get("delta_rotation", 0.0)
                if abs(delta_rot) > 1e-6:
                    try:
                        axis = DB.Line.CreateBound(pt, DB.XYZ(pt.X, pt.Y, pt.Z + 1.0))
                        DB.ElementTransformUtils.RotateElement(doc, current_id, axis, delta_rot)
                        rotate_count += 1
                    except Exception as e:
                        logger.warning("Rotation failed at ({:.1f}, {:.1f}): {}".format(
                            pt.X, pt.Y, str(e)))
