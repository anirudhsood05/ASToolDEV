...
def copy_section_via_reference_lines(source_doc, source_section, dest_doc):
    """Copy section by creating reference model lines that copy correctly, then recreating section."""
    try:
        ...
        # Get section transform and extents
        transform = crop_box.Transform
        min_pt = crop_box.Min
        max_pt = crop_box.Max
        
        # Calculate section line endpoints
        local_p1 = DB.XYZ(min_pt.X, min_pt.Y, 0)
        local_p2 = DB.XYZ(max_pt.X, min_pt.Y, 0)
        world_p1 = transform.OfPoint(local_p1)
        world_p2 = transform.OfPoint(local_p2)
        
        # Create temporary model line in SOURCE document
        temp_line_ids = []
        with revit.Transaction('Create Reference Line', doc=source_doc):
            plane = DB.Plane.CreateByNormalAndOrigin(DB.XYZ.BasisZ, world_p1)
            sketch_plane = DB.SketchPlane.Create(source_doc, plane)
            line = DB.Line.CreateBound(world_p1, world_p2)
            model_line = source_doc.Create.NewModelCurve(line, sketch_plane)
            temp_line_ids.append(model_line.Id)
        
        # Copy to destination - needs transaction for cross-document copy
        cp_options = DB.CopyPasteOptions()
        cp_options.SetDuplicateTypeNamesHandler(CopyUseDestination())
        
        with revit.Transaction('Copy Reference Line', doc=dest_doc):
        ...
        if not copied_line_ids or copied_line_ids.Count == 0:
            with revit.Transaction('Delete Temp Line', doc=source_doc):
                source_doc.Delete(temp_line_ids[0])
            return None
        
        # Get copied line
        copied_line = dest_doc.GetElement(copied_line_ids[0])
        copied_curve = copied_line.GeometryCurve
        p1 = copied_curve.GetEndPoint(0)
        p2 = copied_curve.GetEndPoint(1)
        ...
        # Create section in DESTINATION document
        new_section = None
        with revit.Transaction('Create Section', doc=dest_doc):
            # Calculate directions
            line_dir = (p2 - p1).Normalize()
            view_dir = DB.XYZ(-line_dir.Y, line_dir.X, 0).Normalize()
            
            # Create transform
            sec_transform = DB.Transform.Identity
            sec_transform.Origin = p1
            sec_transform.BasisX = line_dir
            sec_transform.BasisY = DB.XYZ.BasisZ
            sec_transform.BasisZ = view_dir
            
            # Create bounding box
            width = max_pt.X - min_pt.X
            new_box = DB.BoundingBoxXYZ()
            new_box.Transform = sec_transform
            new_box.Min = DB.XYZ(0, min_pt.Y, min_pt.Z)
            new_box.Max = DB.XYZ(width, max_pt.Y, max_pt.Z)
            ...
            # Delete temp line
            dest_doc.Delete(copied_line_ids[0])
        
        # Delete source temp line
        with revit.Transaction('Delete Temp Line', doc=source_doc):
            source_doc.Delete(temp_line_ids[0])
        ...
        logger.error('Error copying section via reference lines: {}'.format(str(e)))
        return None
...
def copy_view(activedoc, source_view, dest_doc):
    ...
    if source_view.ViewType == DB.ViewType.DrawingSheet:
        ...
        try:
            # Use reference line method - model geometry copies correctly
            new_view = copy_section_via_reference_lines(activedoc, source_view, dest_doc)
            
            if new_view:
                print('\t\t\tSection recreated via reference geometry.')
            else:
                logger.error('Failed to recreate section')
...
output.print_md('*Sections will be recreated using reference geometry method*')
output.print_md('')
...
output.print_md('- Models must be linked by **shared coordinates**')
output.print_md('- Section views preserve elevation geometry')
output.print_md('- **Annotations are not transferred** during conversion')
