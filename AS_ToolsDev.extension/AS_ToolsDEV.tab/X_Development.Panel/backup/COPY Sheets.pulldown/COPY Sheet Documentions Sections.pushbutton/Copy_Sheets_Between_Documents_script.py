"""
Copy Sheets Between Documents - FINAL WORKING VERSION
=====================================================
Copies sheets, views, and viewports between open Revit documents.

IMPORTANT LIMITATION:
Elevation views require models to be at same coordinates.
If copying between models at different coordinates, user must:
1. Open destination model
2. Manage > Coordinates > Acquire Coordinates
3. Select source model
4. Run this tool

This aligns the models so elevation markers copy to correct locations.
"""
...
def copy_elevation_markers(activedoc, dest_doc):
    ...
    try:
        ...
        print('WARNING: Could not copy elevation markers. Elevation views will be skipped.')
        return 0
...
def copy_view(activedoc, source_view, dest_doc):
    ...
    if source_view.ViewType == DB.ViewType.DrawingSheet:
        ...
        try:
            logger.debug('Source view is a drafting. Creating destination drafting view.')
        ...
        try:
            logger.debug('Source view is a section/detail. Using direct copy method.')
...
def copy_sheet_viewports(activedoc, source_sheet, dest_doc, dest_sheet):
    """Copy viewports with robust error handling."""
...
def copy_sheet(activedoc, source_sheet, dest_doc):
    """Copy sheet with comprehensive error handling."""
...
# Copy elevation markers if option is enabled
...
if elevations_skipped > 0:
    ...
    output.print_md('  - *Elevation views require plan view markers*')
    output.print_md('  - *For models at different coordinates: Use Manage > Coordinates > Acquire Coordinates before running this tool*')
