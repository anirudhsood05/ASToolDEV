...
__title__ = "Guidance\nPanel"
__author__ = "AUK Digital Practice"
...
xaml_file = """<Window
    ...
    <Grid Margin="12">
        ...
        <Border Grid.Row="0" Background="#F0F0F0" Padding="8" CornerRadius="3">
            <TextBlock Text="Technical Guidance" 
                       FontWeight="Bold" FontSize="14" 
                       HorizontalAlignment="Center"/>
        </Border>

        <!-- Search Section -->
        <GroupBox Grid.Row="2" Header="Search Guidance" Padding="8" Background="White">
            <Grid>
                <Grid.RowDefinitions>
                    <RowDefinition Height="Auto"/>
                    <RowDefinition Height="8"/>
                ...
                <TextBox Name="UI_search_box" Grid.Row="0" Height="25" 
                         VerticalContentAlignment="Center"
                         ToolTip="Search by keywords, RIBA stage, or topic"/>
                
                <StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Right">
                    <Button Name="UI_search_btn" Content="Search" Width="70" Height="25" Margin="0,0,8,0"/>
                    <Button Name="UI_clear_btn" Content="Show All" Width="70" Height="25"/>
                </StackPanel>
            </Grid>
        </GroupBox>

        <!-- Results Section -->
        <GroupBox Grid.Row="4" Header="Guidance Topics" Padding="8" Background="White">
...
class GuidancePanelWindow(forms.WPFWindow):
    ...
    def connect_events(self):
        """Connect UI events."""
        self.UI_search_btn.Click += self.on_search
        self.UI_clear_btn.Click += self.on_show_all
        self.UI_refresh_btn.Click += self.on_refresh
        self.UI_close_btn.Click += self.on_close
        
        # Enter key in search box
        self.UI_search_box.KeyDown += self.on_search_key
    
    def on_window_loaded(self, sender, args):
        """Handle window loaded event."""
        try:
            # Set focus to search box
            self.UI_search_box.Focus()
            print("Window loaded - search box should be focusable")
        except Exception as e:
            print("Error setting focus: {}".format(str(e)))
    
    def on_search_key(self, sender, args):
    ...
    def on_search(self, sender, args):
        ...
        keywords = self.UI_search_box.Text.strip()
        ...
        # Search guidance
        results = self.database.filter_by_context(keywords=keywords)
        self.display_guidance_items(results)
    
    def on_show_all(self, sender, args):
        """Show all guidance items."""
        self.UI_search_box.Text = ""
        self.display_all_guidance()
        self.update_context_label()
...
if __name__ == '__main__':
    try:
        ...
        try:
            ...
            # Get current process (Revit)
            current_process = System.Diagnostics.Process.GetCurrentProcess()
            helper.Owner = current_process.MainWindowHandle
        except:
            # Fallback - window will still work, just without ownership
            pass
