...
<Window
    ...
    <Grid Margin="12">
        ...
        <Border Grid.Row="0" Background="#F0F0F0" Padding="8" CornerRadius="3">
            <TextBlock Text="Aukett Swanke Drawing Package Automation" 
    ...
    def step6_create_sheets(self):
        ...
        for view_data in self.created_views:
            ...
            if use_level_numbering:
                ...
                # Priority 1: Check for basement (B1, LEVEL B1, BASEMENT B2, etc)
                basement_match = re.search(r'B\s*(\d+)', level_name)
                if basement_match:
                    basement_num = int(basement_match.group(1))
                    level_suffix = str(100 - basement_num).zfill(3)
                else:
                    # Priority 2: Standard levels (00, 01, LEVEL 02, etc)
                    # Match digits NOT preceded by 'B'
                    level_match = re.search(r'(?<!B)\b(\d+)\b', level_name)
                    if level_match:
                        level_num = int(level_match.group(1))
                        level_suffix = str(100 + level_num).zfill(3)  # ← FIXED: removed + 1
                    else:
                        # Fallback
